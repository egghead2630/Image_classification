...
# The idea come from some code in internet, the references listed in report
...
class birdDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return image with label derived from the first three characters

    def __len__(self):
        return len(self.data)


def train_model(model, device, criterion, optimizer, scheduler,
                dataloader, train_set, num_epoches=20):
    # The function used to train our model
    # This function refers code on internet,the references listed in the report
    since = time.time()
    # record the time
...
# 1.do the data normalization and augmentation simultaneously and
...
whole_dataset = birdDataset(path=data_path + '/training_labels.txt',
                            transform=transform)
# Define for dataset
...
for i in range(10):
    ...
    train_set, vali_set = torch.utils.data.random_split(whole_dataset,
                                                        [train_size, val_size])
...
# 3. train model
    best_model = train_model(model, device, criterion, optimizer,
                             exp_lr_scheduler, dataload, train_set=train_set)
    path = './best_model'
    index = str(i)
